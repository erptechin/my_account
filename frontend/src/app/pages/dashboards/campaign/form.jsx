...
const doctype = "Platform Integration"
const fields = ['type', 'platform', 'tokens', ]
const subFields = ['add_account','access', 'email']
...
export default function AddEditFrom() {
  ...
  const { id } = useParams();
  const { data: info } = useInfo({ doctype, fields: JSON.stringify([...fields, ...subFields]) });
  const { data, isLoading } = useFeachSingle({ doctype, id, fields: JSON.stringify([...fields, ...subFields]) });
