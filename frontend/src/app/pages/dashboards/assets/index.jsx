// Local Imports
import { Page } from "components/shared/Page";
import { OnboardingItem } from "./OnboardingItem";
import BoyDashboard from "assets/illustrations/boy-dashboard.svg?react";
import GirlDesigning from "assets/illustrations/girl-designing.svg?react";

// ----------------------------------------------------------------------

const items = [
  {
    id: 1,
    title: "Images",
    Image: BoyDashboard,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloribus eius in qui!",
    action: "/dashboards/assets/Image",
    actionLabel: "More...",
  },
  {
    id: 2,
    title: "Videos",
    Image: GirlDesigning,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ea ipsam odio totam?",
    action: "/dashboards/assets/Video",
    actionLabel: "More...",
  }
];

export default function Assets() {
  return (
    <Page title="Assets">
      <div className="transition-content w-full px-(--margin-x) pb-8">
        <div className="py-5 text-center lg:py-6">
          <p className="text-sm uppercase">Assets</p>
          <h3 className="mt-1 text-xl font-semibold text-gray-600 dark:text-dark-100">
            Explore the creations by everyone in this workspace here.
          </h3>
        </div>
        <div className="grid max-w-4xl grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:gap-6 mx-auto">
          {items.map((item) => (
            <OnboardingItem
              key={item.id}
              title={item.title}
              description={item.description}
              action={item.action}
              actionLabel={item.actionLabel}
              Image={item.Image}
            />
          ))}
        </div>
      </div>
    </Page>
  );
}
