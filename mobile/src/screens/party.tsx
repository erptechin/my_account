...
const fields = ['title','transaction_date', 'custom_invoice_option']
...
export default function Party() {
  ...
  const renderComplaint = ({ item }: any) => (
    <HStack className='bg-background-light rounded-lg p-2 my-1 items-center'>
      ...
      <VStack className='pl-2'>
        ...
        <Text className='text-xs text-text-light'>Date:<Text className='text-text-dark'> {(item?.transaction_date)}</Text></Text>
        <Text className='text-xs text-text-light'>Status:<Text className='text-tertiary text-xs'> {item?.custom_invoice_option}</Text></Text>
      </VStack>
    </HStack>
  );
  ...
  return (
    <>
      <MainWapper backgroundColor={'rgb(245, 247, 249)'} refreshing={!isFetched}>
        <Box className='pb-20'>
      ...
      <BottomTabs navigation={navigation} activeTab="Party" />
    </>
  );
}