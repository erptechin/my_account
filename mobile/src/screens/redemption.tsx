...
const fields = ['title','status', 'custom_invoice_option', 'probability']
...
export default function Redemption() {
  ...
  const renderComplaint = ({ item }: any) => (
    <HStack className='bg-background-light rounded-lg p-2 my-1 items-center'>
      ...
      <VStack className='pl-2'>
        ...
        <Text className='text-xs text-text-light'>Probability:<Text className='text-text-dark'> {(item?.probability)}</Text></Text>
  ...
  return (
    <>
      ...
      <BottomTabs navigation={navigation} activeTab="Redemption" />
    </>
  );
}